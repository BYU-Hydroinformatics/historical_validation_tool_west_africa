...
function init_map() {
	...
	var streams = new ol.layer.Image({
		source: new ol.source.ImageWMS({
			url: 'https://tethys2.byu.edu/geoserver/West_Africa-Hydroviewer/wms',
			params: { 'LAYERS': 'africa-west_africa-continental-drainage_line' },
	...
	var stations = new ol.layer.Image({
		source: new ol.source.ImageWMS({
			url: 'https://tethys2.byu.edu/geoserver/West_Africa-Hydroviewer/wms',
			params: { 'LAYERS': 'West_Africa_Stations_v2' },
			serverType: 'geoserver',
			crossOrigin: 'Anonymous'
		})
	});
	...
	map = new ol.Map({
		...
		view: new ol.View({
			center: ol.proj.fromLonLat([-4.996389, 7.238333]),
			zoom: 7
		})
	});
...
let ajax_url = 'https://tethys2.byu.edu/geoserver/West_Africa-Hydroviewer/wfs?request=GetCapabilities';
...
let capabilities = $.ajax(ajax_url, {
	...
	success: function() {
		...
		.split('West_Africa-Hydroviewer:africa-west_africa-continental-drainage_line')[1]
...
function get_discharge_info (watershed, subbasin, streamcomid, stationcode, stationname) {
				...
				$.ajax({
					...
					success: function (result) {
						watershed = 'africa' //OJO buscar como hacerla generica
		         		...
		         		stream = result["features"][0]["properties"]["Stream"];
		         		$("#station-info").append('<h3 id="Watershed-Tab">Watershed: '+ watershed
                        			+ '</h3><h5 id="Subbasin-Tab">Subbasin: '
                        			+ subbasin + '</h3><h5 id="COMID-Tab">Station COMID: '
                        			+ streamcomid);
                        get_discharge_info (watershed, subbasin, streamcomid, stationcode, stationname);
